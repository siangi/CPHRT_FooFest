    ...
    <>
      ...
      <form className='grid grid-col-1 gap-4 lg:gap-8'>
        <div className='flex flex-col border border-white p-6 gap-4'>
          ...
          <P classModifiers='mb-6'>
            Click a campground to check for avaliable spaces at your preferred
            campground. Toilets and hygiene facilites are installed at all
            campgrounds.
          </P>
          <div className='grid row-start-2 w-full h-full'>
            <CampgroundsMap
              value={activeCampground?.area}
              clickFunc={handleMapClick}></CampgroundsMap>
            <div className='text-shade_darker_white break-words'>
              {activeCampground ? (
                <>
                  <P classModifiers='mt-5'>
                    There are{" "}
                    <span className='font-bold text-accent_yellow'>
                      {activeCampground.available}
                    </span>{" "}
                    avaliable spaces at{" "}
                    <span className='font-bold text-accent_yellow'>
                      {activeCampground.area}
                    </span>
                    .
                  </P>
                </>
        ...
        <CheckboxCard
          {...shopData.greenCamping}
          price={shopData.greenCamping.price + " Kr."}
          value={shopData.greenCamping.selected}
          setValue={setCampsGreenly}></CheckboxCard>

        <div className='col-start-1 md:col-start-2 md:row-start-2 w-full flex flex-row justify-end'>
          {formValid ? null : (
            <ErrorP>Please select a campground with enough free spaces</ErrorP>
          )}
