...
function TentForm() {
  ...
  let amountOfTickets = shopData.tickets.reduce(
    ...
    <>
      ...
      <form className='h-full lg:flex-auto flex flex-col gap-3'>
        <div className=' border border-accent_yellow p-6 mb-10'>
          <H4 classModifiers='text-shade_darker_white font-bold mb-4'>
            You have booked tickets for {amountOfTickets} person/people.
          </H4>
          <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
            <P>
              At Ragnarock, it is mandatory to have a tent to sleep in - this
              counts for all campgrounds. We are happy to offer you the option
              to rent a tent - either a tent for two or three people.
            </P>
            <P>
              If you choose the to rent a tent from Ragnarock, you can leave all
              your tent-related stress at home - we will build it for you!{" "}
              <span className='text-accent_yellow'>
                The important thing everyone has a designated spot in their own
                tent.
              </span>
            </P>
          </div>
          ...
          return (
            <OptionCard
              ...
              updateAmount={(newAmount) =>
                updateAmount(tentOption.id, newAmount)
              }
              initialAmount={tentOption.amountOfTents}></OptionCard>
          );
        })}
        <div className='flex flex-row gap-4 justify-end'>
          {formValid ? null : (
            <ErrorP>
              Your currently only have space for {getAmountOfSpace()} out of{" "}
              {amountOfTickets} People!
            </ErrorP>
          )}
          <SecondaryButton
            caption='Bring own Tents'
            action={noTents}></SecondaryButton>
          <PrimaryButton
            caption='Confirm Selection'
            action={submitTentForm}></PrimaryButton>
        </div>
      </form>
    </>
  );
